...
public class UrlValidatorTest extends TestCase {
   ...
   public void testManualTest()
   {
	   ...
	   do {
		   System.out.print("Enter a url ('exit' to quit): ");
		   String url = userInput.next();
		   if(url.contentEquals("exit")) {
			   System.out.println("Manual testing complete");
			   break;
		   }
		   System.out.print("Should this test pass or fail? (true/false): ");
		   boolean isValid = userInput.nextBoolean();
		   ResultPair rp = new ResultPair(url, isValid);
		   System.out.println("Result: " + urlVal.isValid(rp.item));
	   }while(true);
   ...
   private ArrayList<String> getTLDList()
   {
	   ArrayList<String> TLDs = new ArrayList<String>();
	   File urlFile = new File("./TLDList.txt"); //connect to file
	   try { //try to create scanner
			Scanner sc = new Scanner(urlFile);
			while(sc.hasNextLine()){ //scan through file
				String line = sc.nextLine(); //grab each url
				TLDs.add(line);
			}
   ...
   private String getRandomString(int len, String validCharSet) {
		Random rng = new Random();
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < len; i++) {
			sb.append(validCharSet.charAt(rng.nextInt(validCharSet.length())));
		}
   ...
   public void testYourFirstPartition()
   {
	   ...
	   System.out.println("Missing TLDs: " + missingTLDCount);
   }
   //Negative Tests
...
private void testRandomInvalidURL(List<String> tldList) {
	//setup options
	long options = 
		UrlValidator.ALLOW_2_SLASHES   +
		UrlValidator.ALLOW_ALL_SCHEMES +
	...
	// 0. Scheme 1. Domain 2. TLD 3. Port
	int invalidSection = rng.nextInt(4);
	
	StringBuilder URL = new StringBuilder();
	if (invalidSection == 0) {
		URL.append(generateRandomInvalidScheme());
	}
	else {
		URL.append(generateRandomValidScheme());
	}
	
	String TLD;
	if (invalidSection == 2) {
		TLD = generateRandomInvalidTLD();
	}
	else {
		TLD = generateRandomValidTLD();
		if (!tldList.contains(TLD.toLowerCase())) {
			System.out.println("Missing TLD: " + TLD);
			missingTLDCount++;
			return;
		}
	}
	...
	if (invalidSection == 1) {
		URL.append(generateRandomDomain(TLD, false));
	}
	else {
		URL.append(generateRandomDomain(TLD, true));
	}
	 
	//Generate Port
	if (invalidSection == 3) {
		URL.append(generateRandomInvalidPort());
	}
	else if (rng.nextBoolean()) {
		URL.append(generateRandomValidPort());
	}
	
	//Generate Path
	if (rng.nextBoolean()) {
		URL.append(generateRandomValidPath());
	}
	 
	//Generate Query
	if (rng.nextBoolean()) {
		URL.append(generateRandomValidQuery());
	}
	 
	 
	if (urlVal.isValid(URL.toString())) {
		System.out.println(URL.toString());
	}
	assertFalse(urlVal.isValid(URL.toString()));
}


private void testRandomValidURL(List<String> tldList) {
	//setup options
	long options = 
		UrlValidator.ALLOW_2_SLASHES   +
		UrlValidator.ALLOW_ALL_SCHEMES +
	...
	StringBuilder URL = new StringBuilder(generateRandomValidScheme());
	
	String TLD = generateRandomValidTLD();
	if (!tldList.contains(TLD.toLowerCase())) {
		System.out.println("Missing TLD: " + TLD);
		missingTLDCount++;
		return;
...
private String generateRandomValidQuery() {
	Random rng = new Random();
	StringBuilder URL = new StringBuilder();
	URL.append('?');
	String validQueryChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
	
	int numQueries = rng.nextInt(20) + 1;
	
	for(int i = 0; i < numQueries; i++) {
		URL.append(getRandomString(rng.nextInt(100) + 1, validQueryChars));
		URL.append('=');
		URL.append(getRandomString(rng.nextInt(100) + 1, validQueryChars));
		URL.append('&');
	}
...
private String generateRandomValidPort() {
	Random rng = new Random();
	return ":" + rng.nextInt(65536);
}

private String generateRandomInvalidPort() {
	Random rng = new Random();
	return ":" + rng.nextInt(1000000) + 65536;
}

private String generateRandomDomain(String TLD, boolean validFlag) {
	Random rng = new Random();
	
	//Generate Labels
	String validLabelChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-";
	String invalidLabelChars = "!@#$%^&*()_+=;'><*~`{}][|";
	
	String charSet;
	int labelMax = 63;
	int totalMax = 253;
	if (validFlag) {
		charSet = validLabelChars;
	}
	else {
		charSet = invalidLabelChars;
		labelMax = 70;
		totalMax = 300;
	}
	
	int numLabels = rng.nextInt(10) + 1;
	
	StringBuilder URL = new StringBuilder();
	
	//Generate Label Sizes
	int[] labelSizes = new int[numLabels];
	int totalSize;
	do {
		totalSize = 0;
		for (int i = 0; i < numLabels; i++) {
			labelSizes[i] = rng.nextInt(labelMax) + 1;
			totalSize += labelSizes[i] + 1;
		}
	} while(totalSize > totalMax - TLD.length());
	
	//Populate Labels
	for (int i = 0; i < numLabels; i++) {
		for (int j = 0; j < labelSizes[i]; j++) {
			if (j != labelSizes[i] - 1 && j != 0 ) {
				URL.append(charSet.charAt(rng.nextInt(charSet.length())));
			} else {
				URL.append(charSet.charAt(rng.nextInt(charSet.length() - 1)));
			}
		}
		URL.append(".");
	}
...
private String generateRandomValidTLD() {
	Random rng = new Random();
	ArrayList<String> TLDList = getTLDList();
	
	//Generate TLD
	String TLD = TLDList.get(rng.nextInt(TLDList.size()));
	
	return TLD;
}

private String generateRandomInvalidTLD() {
	ArrayList<String> TLDList = getTLDList();
	Random rng = new Random();
...
private String generateRandomValidScheme() {
	//Generate random Scheme
	Random rng = new Random();
	String[] validSchemes = {"http://", "https://", "ftp://"};
	return validSchemes[rng.nextInt(validSchemes.length)];
}
   
private String generateRandomInvalidScheme() {
	//Generate random Scheme
	Random rng = new Random();
	String[] invalidSchemes = {"http//", "https:/", "ftp", "", "fubar", "://"};
	return invalidSchemes[rng.nextInt(invalidSchemes.length)];
}
   ...
   public void testIsValid()
   {
	   ...
	   //create resultpair list
	   ArrayList<ResultPair> testingURLs = new ArrayList<ResultPair>();
	   File urlFile = new File("./testingURLs.txt"); //connect to file
		try { //try to create scanner
			Scanner sc = new Scanner(urlFile);
			while(sc.hasNextLine()){ //scan through file
				String line = sc.nextLine(); //grab each url
			...
			sc.close(); //close scanner
			for(ResultPair rp: testingURLs) { //loop through array
				System.out.println(rp.item);
				if(rp.valid) {
					assertTrue(urlVal.isValid(rp.item)); //assert true or false depending on the expected result
				}else {
					assertFalse(urlVal.isValid(rp.item));
				}
			}
		} catch (FileNotFoundException e) { //catch exceptions
			e.printStackTrace(); //print trace
   ...
   private static final String[] TESTING_TLDS = new String[] {
